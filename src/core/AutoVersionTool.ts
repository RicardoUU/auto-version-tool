...
import { CommitInfo, Config, RunOptions, VersionInfo } from "../types/Config";
...
export class AutoVersionTool {
  ...
  async run(options: RunOptions): Promise<void> {
    ...
    // 获取自上次标签以来的提交
    const commits = await this.gitService.getCommitsSinceLastTag(
      options.branch
    );
    ...
    console.log(
      chalk.blue(
        `🎯 版本将从 ${versionInfo.current} 升级到 ${versionInfo.next} (${versionInfo.type})`
      )
    );
  ...
  private async determineVersionBump(
    commits: CommitInfo[],
    versionType: string
  ): Promise<VersionInfo> {
  ...
  private async showPreview(
    ...
    console.log("".padEnd(50, "-"));
    console.log(
      chalk.blue(`版本变更: ${versionInfo.current} → ${versionInfo.next}`)
    );
    console.log(chalk.blue(`变更类型: ${versionInfo.type}`));
    console.log(chalk.blue(`影响提交: ${commits.length} 个`));

    // 显示将要生成的changelog预览
    const changelogPreview = await this.changelogService.generateChangelog(
      versionInfo,
      commits
    );
    console.log("\n📝 Changelog 预览:");
    console.log("".padEnd(50, "-"));
    console.log(
      changelogPreview.substring(0, 500) +
        (changelogPreview.length > 500 ? "..." : "")
    );
  }
